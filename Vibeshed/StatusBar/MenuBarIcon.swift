...
enum MenuBarIcon {
    static func make(size: CGFloat = 18, capsLockActive: Bool = false) -> NSImage {
        let image = NSImage(size: NSSize(width: size, height: size), flipped: false) { rect in
            ...
            let headCenter = CGPoint(x: width * 0.44, y: height * 0.54)
            let headRadius = min(width, height) * 0.16
            drawWand(
                headCenter: headCenter,
                headRadius: headRadius,
                tipCenter: CGPoint(x: width * 0.18, y: height * 0.18),
                bodyHalfWidth: min(width, height) * 0.085
            )

            // --- Concentric waves on the upper-right of the head ---
            guard let ctx = NSGraphicsContext.current?.cgContext else { return false }
            let strokeWidth = max(1.0, min(width, height) * 0.055)
            drawWaves(in: ctx, around: headCenter, headRadius: headRadius, strokeWidth: strokeWidth)

            if capsLockActive {
                let dotRadius = min(width, height) * 0.1
                let dotCenter = CGPoint(x: width * 0.85, y: height * 0.15)
    ...
    /// The wand: an angled capsule body from the tip to the bulb head, with the head on top.
    private static func drawWand(
        headCenter: CGPoint,
        headRadius: CGFloat,
        tipCenter: CGPoint,
        bodyHalfWidth: CGFloat
    ) {
